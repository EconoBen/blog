...
import { countTopics, topicDescription } from '../lib/topics';
...
export default async function PostsPage() {
  ...
  const topTags = countTopics(posts).slice(0, 4);
    ...
    <EditorialPageFrame currentPath="/posts">
      ...
      <section className="border-t border-outline-variant/20 pt-4 pb-12 md:pt-5 md:pb-16">
        <div className="mx-auto max-w-[1440px] px-5 md:px-8">
          <PostsList posts={serializedPosts} latestSlug={posts[0]?.slug} />
        </div>
      </section>
            ...
            {topTags.map(({ tag, count }) => (
              <article key={tag} className="group overflow-hidden sticky-note p-6 transition-transform duration-300 hover:-translate-y-1">
                <div className="space-y-4">
                  ...
                  <p className="font-body text-sm leading-relaxed text-on-surface">{topicDescription(tag)}</p>
