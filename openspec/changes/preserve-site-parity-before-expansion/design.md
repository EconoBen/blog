...
> Status note (April 12, 2026, direct human design session): a substantial interactive design session with the user covered `/about`, `/talks`, `/`, `/posts`, `/tags`, `/tags/[tag]`, and site-wide UX. Key outcomes: about page switched from dark Monograph to light editorial theme with full CV content, photo-in-hero layout, Option F inline stat highlights, and forward-looking focus cards. Talks page fixed pre-existing hydration mismatch, added live Spotify embeds, dates on all cards. Home page replaced stats bar with focus cards, featured post card made fully clickable. Site-wide: title changed to Staff AI/ML Engineer, hover affordance audit across all pages, tag dedup, blue text bleed fix on card links, consistent spacing. 8 commits pushed to `origin/feat/site-current-review`. See tasks.md section 14 for full itemized progress.

> Status note (April 2, 2026, current-review posts fold refinement): a further screenshot-led pass on `http://127.0.0.1:3018/posts` addressed the remaining “plain and awkward” feel in the latest-post card. Verified desktop and mobile screenshots under `.playwright-current-review-20260402-posts-fix11/` show the updated fold: the latest-post card now uses a split feature treatment with a tinted title panel and a separate detail column, so the card finally uses its horizontal space instead of reading like a wide blank slab. `/posts` remains open, but the first-screen composition is no longer the obvious weak point it was before.

> Status note (March 30, 2026, current-review parallel agent pass): work resumed on the integrated review branch `feat/site-current-review` instead of the older stitched preview branches. The review surface is now a production preview on `http://127.0.0.1:3018`, with route-owned improvement passes and fresh Playwright screenshots under `.playwright-current-review-20260330-audit/` and `.playwright-current-review-20260330-pass2/`. This pass materially changed the blocker profile: `/about` regained the missing CV/resume depth, `/talks` is calmer and more archive-like at the top, `/code-ai` now opens with real entries and reads like an intentional archive instead of a softened dashboard, and the shared shell is slightly quieter. The honest ranking after that pass is `/about`, `/code-ai`, `/talks`, then `/posts`; `/posts` remains the weakest route because the lead composition is still awkward, the featured surface is underfilled, and the year-ledger rhythm below still repeats with too much sameness and whitespace.
